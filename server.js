...
<script>
  ...
  function render(data) {
    ...
    rows.forEach(function(row, idx) {
      ...
      var safeLink = row.link && /^https?:\/\//i.test(row.link) ? row.link : null;
      var fileHtml = safeLink
        ? '<a href="' + esc(safeLink) + '" target="_blank" rel="noopener noreferrer">' + esc(row.file) + '</a>'
        : esc(row.file);
...
const server = http.createServer((req, res) => {
  ...
  if (req.method === 'GET' && p === '/api/review') {
    ...
    if (!file || path.basename(file) !== file || !/^.+-pr-\d+\.md$/.test(file)) { res.writeHead(400); return res.end('Bad request'); }
